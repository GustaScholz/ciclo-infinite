...
function Admin() {
  ...
  function editProduct(product) {
    ...
    const category = PRODUCT_CATEGORIES.includes(product.category) ? product.category : PRODUCT_CATEGORIES[0];
  ...
  function saveDraft(event) {
    ...
    if (!form.name.trim() || !PRODUCT_CATEGORIES.includes(form.category) || Number.isNaN(price)) {
    ...
    const product = {
      ...
      price,
      sizes: form.sizes.filter((size) => form.sizeOptions.includes(size)),
  ...
  async function uploadImages(files) {
    if (!token) {
      setStatus("Conecte a chave do GitHub antes de enviar fotos.");
      return;
    }
  ...
  async function publish() {
    if (!token) {
      setStatus("Conecte sua chave do GitHub para publicar.");
      return;
    }
    setBusy(true);
    ...
    try {
      ...
      await github(`/repos/${OWNER}/${REPO}/contents/${DATA_PATH}`, token, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          message: "Atualiza catálogo pelo painel administrativo",
  ...
  return (
    <div className="min-h-screen bg-zinc-100 text-zinc-950">
      ...
      <main className="site-container py-8 sm:py-10">
        ...
        {status && activeTab === "products" && <div className="mb-7 rounded-2xl bg-zinc-900 text-white px-5 py-4 text-sm">{status}</div>}

        <div className="flex gap-2 mb-7 border-b border-zinc-300">
          <button onClick={() => setActiveTab("products")} className={`px-5 py-3 font-bold border-b-2 transition ${activeTab === "products" ? "border-black text-black" : "border-transparent text-zinc-400 hover:text-black"}`}>Produtos</button>
          <button onClick={() => setActiveTab("banners")} className={`px-5 py-3 font-bold border-b-2 transition ${activeTab === "banners" ? "border-black text-black" : "border-transparent text-zinc-400 hover:text-black"}`}>Banners</button>
        </div>

        {activeTab === "banners" ? (
          <AdminBanners token={token} />
        ) : (
          <div className="grid grid-cols-1 xl:grid-cols-[430px_1fr] gap-8 items-start">
            <section className="bg-white rounded-3xl p-5 sm:p-6 shadow-sm xl:sticky xl:top-24">
              ...
              <form onSubmit={saveDraft} className="space-y-4">
                <label className="block"><span className="text-sm font-semibold">Nome</span><input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="mt-2 w-full border rounded-xl px-4 h-11 outline-none focus:border-black" /></label>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <label className="block"><span className="text-sm font-semibold">Preço</span><input value={form.price} onChange={(event) => setForm({ ...form, price: event.target.value })} className="mt-2 w-full border rounded-xl px-4 h-11 outline-none focus:border-black" placeholder="89,90" /></label>
                  <label className="block">
                    <span className="text-sm font-semibold">Categoria</span>
                    <select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} className="mt-2 w-full border rounded-xl px-4 h-11 outline-none focus:border-black bg-white">
                      {PRODUCT_CATEGORIES.map((category) => <option key={category} value={category}>{category}</option>)}
                    </select>
                  </label>
                </div>

                <label className="block"><span className="text-sm font-semibold">Descrição</span><textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} rows="4" className="mt-2 w-full border rounded-xl px-4 py-3 outline-none focus:border-black resize-none" /></label>

                <div>
                  <p className="text-sm font-semibold">Tamanhos da peça</p>
                  <div className="flex flex-wrap gap-2 mt-2">{DEFAULT_SIZES.map((size) => <button type="button" key={size} onClick={() => toggleSizeOption(size)} className={`px-3 py-2 rounded-full border text-sm ${form.sizeOptions.includes(size) ? "bg-black text-white border-black" : "border-zinc-300"}`}>{size}</button>)}</div>
                  <div className="flex gap-2 mt-3"><input value={customSize} onChange={(event) => setCustomSize(event.target.value)} placeholder="Outro tamanho" className="flex-1 border rounded-xl px-3 h-10 outline-none focus:border-black" /><button type="button" onClick={addCustomSize} className="border border-black rounded-xl px-4 text-sm font-semibold">Adicionar</button></div>
                </div>

                <div>
                  <p className="text-sm font-semibold">Disponíveis em estoque</p>
                  <p className="text-xs text-zinc-500 mt-1">Desmarcado = tamanho aparece bloqueado para o cliente.</p>
                  <div className="flex flex-wrap gap-2 mt-3">{form.sizeOptions.map((size) => <button type="button" key={size} onClick={() => toggleAvailable(size)} className={`px-4 py-2 rounded-full border text-sm ${form.sizes.includes(size) ? "bg-emerald-600 text-white border-emerald-600" : "bg-zinc-100 text-zinc-400 border-zinc-200 line-through"}`}>{size}</button>)}</div>
                </div>

                <div>
                  <div className="flex items-center justify-between"><p className="text-sm font-semibold">Fotos</p><label className={`inline-flex items-center gap-2 text-sm font-semibold ${token ? "cursor-pointer" : "opacity-40 cursor-not-allowed"}`}><ImagePlus size={18} /> Adicionar<input type="file" accept="image/*" multiple disabled={!token || busy} onChange={(event) => uploadImages(event.target.files)} className="hidden" /></label></div>
                  <div className="grid grid-cols-3 gap-2 mt-3">
                    {form.images.map((image, index) => <div key={`${image}-${index}`} className="relative aspect-[3/4] bg-zinc-100 rounded-xl overflow-hidden"><img src={`${import.meta.env.BASE_URL}${image.replace(/^\//, "")}`} alt="" className="w-full h-full object-cover" /><button type="button" onClick={() => setForm((current) => ({ ...current, images: current.images.filter((_, imageIndex) => imageIndex !== index) }))} className="absolute top-1 right-1 bg-white/90 w-7 h-7 rounded-full text-xs font-bold">✕</button></div>)}
                    {form.images.length === 0 && <div className="col-span-3 border border-dashed rounded-xl py-8 text-center text-sm text-zinc-400">Nenhuma foto adicionada</div>}
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.isNew} onChange={(event) => setForm({ ...form, isNew: event.target.checked })} /> Novidades</label>
                  <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /> Visível no site</label>
                </div>
            ...
            <section>
              <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-5">
                <div><p className="text-xs uppercase tracking-[3px] text-zinc-500">Catálogo</p><h2 className="text-3xl font-black mt-1">{products.length} produtos</h2><p className="text-zinc-500 mt-1">Edite ou remova peças daqui.</p></div>
                <button disabled={!dirty || busy || !token} onClick={publish} className="bg-emerald-600 text-white rounded-xl px-5 h-12 font-bold flex items-center justify-center gap-2 disabled:bg-zinc-300 disabled:text-zinc-500"><UploadCloud size={20} /> {busy ? "Aguarde..." : "Publicar alterações"}</button>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {sortedProducts.map((product) => (
                  <article key={product.id} className={`bg-white rounded-2xl p-4 shadow-sm flex gap-4 ${product.active === false ? "opacity-50" : ""}`}>
                    <div className="w-24 h-32 bg-zinc-100 rounded-xl overflow-hidden shrink-0">
                      {product.images?.[0] || product.image ? <img src={`${import.meta.env.BASE_URL}${(product.images?.[0] || product.image).replace(/^\//, "")}`} alt={product.name} className="w-full h-full object-cover" /> : <div className="w-full h-full grid place-items-center text-xs text-zinc-400">Sem foto</div>}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs uppercase tracking-[2px] text-zinc-500 truncate">{product.category}</p>
                      <h3 className="font-bold mt-1 leading-tight break-words">{product.name}</h3>
                      <p className="font-semibold mt-1">R$ {Number(product.price).toFixed(2).replace(".", ",")}</p>
                      <p className="text-xs text-zinc-500 mt-2">Disponíveis: {(product.sizes || []).join(", ") || "nenhum"}</p>
                      <div className="flex items-center gap-2 mt-3"><button onClick={() => editProduct(product)} className="w-9 h-9 rounded-full bg-zinc-100 grid place-items-center"><Edit3 size={17} /></button><button onClick={() => removeProduct(product)} className="w-9 h-9 rounded-full bg-red-50 text-red-600 grid place-items-center"><Trash2 size={17} /></button></div>
                    </div>
                  </article>
                ))}
              </div>
